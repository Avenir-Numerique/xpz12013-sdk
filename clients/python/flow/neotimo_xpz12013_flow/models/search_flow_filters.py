...
class SearchFlowFilters(BaseModel):
    """
    Filtering criteria, at least one is required
    """ # noqa: E501
    updated_after: Optional[datetime] = Field(default=None, description="The comparison with current date is strict : updatedAt > updatedAfter", alias="updatedAfter")
    updated_before: Optional[datetime] = Field(default=None, alias="updatedBefore")
    processing_rule: Optional[List[ProcessingRule]] = Field(default=None, alias="processingRule")
    flow_type: Optional[List[FlowType]] = Field(default=None, alias="flowType")
    flow_direction: Optional[List[FlowDirection]] = Field(default=None, alias="flowDirection")
    tracking_id: Optional[Annotated[str, Field(strict=True, max_length=64)]] = Field(default=None, description="Unique identifier supporting UUID but not only, for flexibility purpose", alias="trackingId")
    ack_status: Optional[FlowAckStatus] = Field(default=None, alias="ackStatus")
