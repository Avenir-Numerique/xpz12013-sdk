...
class RoutingCodeSearch(BaseModel):
    """
    RoutingCodeSearch
    """ # noqa: E501
    filters: Optional[RoutingCodeSearchFilters] = None
    sorting: Optional[List[RoutingCodeSearchSortingInner]] = Field(default=None, description="Sorting criteria on a field and an order (ascending or descending).")
    fields: Optional[List[RoutingCodeField]] = Field(default=None, description="Allows you to filter the desired fields in the response.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "filters": RoutingCodeSearchFilters.from_dict(obj["filters"]) if obj.get("filters") is not None else None,
            "sorting": [RoutingCodeSearchSortingInner.from_dict(_item) for _item in obj["sorting"]] if obj.get("sorting") is not None else None,
