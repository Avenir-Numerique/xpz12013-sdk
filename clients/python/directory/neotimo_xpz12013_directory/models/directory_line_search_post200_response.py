...
class DirectoryLineSearchPost200Response(BaseModel):
    """
    DirectoryLineSearchPost200Response
    """ # noqa: E501
    search: Optional[SearchDirectoryLine] = None
    total_number_of_results: Optional[StrictInt] = Field(default=-1, description="The total number of results, if -1 is returned then this value is not managed due to performance issue", alias="totalNumberOfResults")
    results: Optional[List[DirectoryLinePayloadStatusLegalUnitFacilityRoutingCode]] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "search": SearchDirectoryLine.from_dict(obj["search"]) if obj.get("search") is not None else None,
            "totalNumberOfResults": obj.get("totalNumberOfResults") if obj.get("totalNumberOfResults") is not None else -1,
            "results": [DirectoryLinePayloadStatusLegalUnitFacilityRoutingCode.from_dict(_item) for _item in obj["results"]] if obj.get("results") is not None else None
        })
        return _obj
