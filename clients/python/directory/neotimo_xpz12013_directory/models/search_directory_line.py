...
class SearchDirectoryLine(BaseModel):
    """
    SearchDirectoryLine
    """ # noqa: E501
    filters: Optional[SearchDirectoryLineFilters] = None
    sorting: Optional[List[SearchDirectoryLineSortingInner]] = Field(default=None, description="Sorting criteria on a field and an order (ascending or descending).")
    fields: Optional[List[DirectoryLineField]] = Field(default=None, description="Allows you to filter the desired fields in the response.")
    ...
    def include_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['siren', 'siret', 'routingCode']):
                raise ValueError("each list item must be one of ('siren', 'siret', 'routingCode')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "filters": SearchDirectoryLineFilters.from_dict(obj["filters"]) if obj.get("filters") is not None else None,
            "sorting": [SearchDirectoryLineSortingInner.from_dict(_item) for _item in obj["sorting"]] if obj.get("sorting") is not None else None,
