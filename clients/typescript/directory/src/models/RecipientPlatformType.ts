...
/**
 * The type of platform for document reception (from UNCL 3035 list) :
 *   - WK : Value added network provider (PA)
 *   - DFH : Government service requestor (PPF)
 * 
 * @export
 */
export const RecipientPlatformType = {
    Wk: 'WK',
    Dfh: 'DFH'
} as const;
export type RecipientPlatformType = typeof RecipientPlatformType[keyof typeof RecipientPlatformType];


export function instanceOfRecipientPlatformType(value: any): boolean {
    for (const key in RecipientPlatformType) {
        if (Object.prototype.hasOwnProperty.call(RecipientPlatformType, key)) {
            if (RecipientPlatformType[key as keyof typeof RecipientPlatformType] === value) {
                return true;
            }
        }
    }
...
export function RecipientPlatformTypeFromJSON(json: any): RecipientPlatformType {
    return RecipientPlatformTypeFromJSONTyped(json, false);
}

export function RecipientPlatformTypeFromJSONTyped(json: any, ignoreDiscriminator: boolean): RecipientPlatformType {
    return json as RecipientPlatformType;
}

export function RecipientPlatformTypeToJSON(value?: RecipientPlatformType | null): any {
    return value as any;
}

export function RecipientPlatformTypeToJSONTyped(value: any, ignoreDiscriminator: boolean): RecipientPlatformType {
    return value as RecipientPlatformType;
}
