...
```powershell
git tag v1.3.0          # version alignée sur la norme XP Z12-013
git push origin v1.3.0
```

> **Versionnage** : le SDK est versionné sur la **version de norme couverte** (ici 1.3.0).
> Cette version introduit des ruptures d'API (webhooks aplatis, opérations webhook retirées,
> valeurs d'enum modifiées) malgré l'incrément mineur — voir `CHANGELOG.md` pour le guide de migration.
